...
| 도메인 | 역할 | 주요 책임 |
|--------|------|------------|
| **Product**<br>(상품) | 사용자가 구매할 수 있는 제품 정보를 제공 | - 상품 목록/상세 조회<br>- 이름, 가격, 설명, 옵션 관리<br>- 외부에선 읽기 전용 |
| **Inventory**<br>(재고) | 각 상품의 구매 가능한 수량 관리 | - 재고 수량 확인<br>- 재고 확보 및 차감<br>- 상품 ID로 연결됨 |
| **Order**<br>(주문) | 사용자의 구매 의사를 표현 | - 주문 생성<br>- 주문 상태 관리<br>- 결제 및 통계 연계 |
| **Payment**<br>(결제) | 실제 결제를 수행하고, 정책을 적용 | - 포인트 차감 요청<br>- 쿠폰 사용 처리<br>- 결제 성공 시 주문 상태 변경<br>- 통계 전송 트리거 |
| **Point**<br>(포인트) | 사용자의 충전 금액을 관리 | - 포인트 충전/사용<br>- 잔액 조회<br>- 포인트 이력 기록 |
| **Coupon**<br>(쿠폰) | 할인 혜택을 발급/적용 | - 쿠폰 정의 및 발급<br>- 사용자 보유 쿠폰 관리<br>- 유효성 검증 및 사용 처리 |
